...
package demo.j_sample_fname_0_1;
...
/**
 * Job: j_sample_fname Purpose: For demo of GIT pull & push<br>
 * Description: To create a job for demonstrating GIT pull & push option of
 * Talend Studio <br>
 * 
 * @author C, Toronto
 ...
 * @status
 */
public class j_sample_fname implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "j_sample_fname.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager
			.getLogger(j_sample_fname.class);
	...
	private final String jobName = "j_sample_fname";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					j_sample_fname.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(j_sample_fname.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		final static byte[] commonByteArrayLock_DEMO_j_sample_fname = new byte[0];
		static byte[] commonByteArray_DEMO_j_sample_fname = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_DEMO_j_sample_fname.length) {
					if (length < 1024 && commonByteArray_DEMO_j_sample_fname.length == 0) {
						commonByteArray_DEMO_j_sample_fname = new byte[1024];
					} else {
						commonByteArray_DEMO_j_sample_fname = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_DEMO_j_sample_fname, 0, length);
				strReturn = new String(commonByteArray_DEMO_j_sample_fname, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_DEMO_j_sample_fname, 0, length);
				strReturn = new String(commonByteArray_DEMO_j_sample_fname, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_DEMO_j_sample_fname) {
...
///////////////////////		
					...
					if (row1.Fname != null) { //
						...
						strBuffer_tLogRow_1.append(String.valueOf(row1.Fname));
...
///////////////////////    			
	...
	public static void main(String[] args) {
		final j_sample_fname j_sample_fnameClass = new j_sample_fname();

		int exitCode = j_sample_fnameClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'j_sample_fname' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'j_sample_fname' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = j_sample_fname.class.getClassLoader()
					.getResourceAsStream("demo/j_sample_fname_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = j_sample_fname.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : j_sample_fname");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 53465 characters generated by Talend Cloud Data Management Platform on the
 * February 16, 2022 at 11:53:43 PM EST
 ************************************************************************************************/